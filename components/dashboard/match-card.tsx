...
export function MatchCard({ match, onScheduleChat }: MatchCardProps) {
  const { user } = match

  return (
    <Card className="h-full border-border/70 bg-card/80 backdrop-blur">
      <CardHeader className="flex flex-row items-center gap-4">
        <Avatar src={user.profilePhotoUrl ?? undefined} fallback={user.fullName.split(" ").map((part) => part[0]).join("").slice(0, 2).toUpperCase()} />
        <div>
      ...
      <CardFooter className="flex gap-2">
        <Button className="flex-1" onClick={() => onScheduleChat?.(user.userId)}>
          <Coffee className="mr-2 h-4 w-4" /> Suggest coffee chat
        </Button>
        <Button variant="ghost" className="flex-1" onClick={() => onScheduleChat?.(user.userId)}>
